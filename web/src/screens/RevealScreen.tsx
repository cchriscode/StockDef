...
export function RevealScreen({ sessionId, finish, regionId, onDone }: Props) {
  ...
  useEffect(() => {
    ...
    const up = colorBlind ? '#e8c34a' : '#ef5350';
    const down = colorBlind ? '#4aa8e8' : '#42a5f5';
  ...
  if (step >= 6) {
    // 정산 카드 + 점령 보상 선택 (FR-8.4~8.6)
    return (
      <div className="screen center reveal-final">
        <h2>{failed ? '패배 — 다시 도전하세요' : `클리어! 등급 ${finish.grade}`}</h2>
    ...
    <div className="screen center reveal">
      ...
      {step >= 0 && (
        <div className="reveal-step">
          <canvas ref={dailyRef} width={640} height={150} />
      ...
      {step >= 3 && (
        <div className="reveal-step">
          <canvas ref={posRef} width={640} height={80} />
      ...
      {step >= 5 && !failed && !isTut && (
        <div className={`codex-card rarity-${data.rarity}`}>
          [차트 도감 등록] {data.ticker} · {RARITY_KO[data.rarity] ?? data.rarity}
        </div>
      )}
