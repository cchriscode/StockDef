...
export const BALANCE = {
  ...
  // 2026-08-10 조임: 체력 곡선 완만화로 이지가 R1 75%·R3 60%까지 헐거워져 한 단계 올렸다.
  // 골드를 늘리는 방식은 +100%를 줘도 10→13%에 그쳐(전열이 먼저 무너짐) 스탯 완화로 간다.
  // 2026-08-10 재조정: 적이 사옥 앞에 쌓여 계속 싸우게 되면서(FR-6.11) 압력이 올라 한 단계 더 완화.
  EASY_HP_MULT: 0.5, // 이지: 하드 대비 −50%
  EASY_DPS_MULT: 0.55, // 이지: 하드 대비 −45%
  EASY_COUNT_MULT: 0.75, // 이지: 적 수 −25%
...
// 주기형 스킬의 재사용 간격(초). 교전 누적(ENEMY_SKILL_HITS)이 있는 근접·사격형은 이 표를 쓰지 않는다.
// 설명은 ENEMY_SKILL의 현행 효과 기준 (구 스킬 이름이 남아 있어 2026-08-10 정정)
export const ENEMY_SKILL_PERIOD: Record<EnemyTypeSpec['key'], number> = {
  grunt: 9, // (미사용 — 교전 누적형)
  runner: 8, // (미사용 — 교전 누적형)
  shield: 12, // (미사용 — 교전 누적형)
  tank: 11, // 융단 포격 — 두 지점 광역 (dps×0.8)
